...
# 文件夹路径
folder_path = 'F:\教育数据集\java\metadata'
...
for filename in os.listdir(folder_path):
    ...
    if filename.endswith('.csv'):
        ...
        if 'filename_ext' in df.columns:  # 检查'filename_ext'列是否存在
            filtered_df = df[df['filename_ext'] == 'java']  # 筛选出filename_ext字段值为'py'的行
...
# 文件夹路径
folder_path1 = 'F:\\教育数据集\\java\\java'
...
# 检查文件是否存在
file_path_code_contents= 'data/Afterprocess/problems_code_contents_codenet_java.pkl'
if os.path.exists(file_path_code_contents):
    ...
    with open('data/Afterprocess/problems_code_contents_codenet_java.pkl', 'wb') as f:
        ...
        if filename.endswith('.html'):
            ...
            if file_name_without_extension2 in raw_question:
                with open(file_path, 'r', encoding='utf-8') as file:
                    ...
                    # print("\n")
# 检查文件是否存在
file_path_question_contents= './data/questions_num2questiondict_temp_codenet_java.pkl'
if os.path.exists(file_path_question_contents):
    ...
    # 如果文件不存在，则运行函数 search_files(folder_path1)
    folder_path2 = 'F:\教育数据集\java\problem_descriptions'
    getquestion_content(folder_path2)
        # 将字典存储到本地文件
    with open('./data/Afterprocess/questions_num2questiondict_temp_codenet_java.pkl', 'wb') as f:
...
# 将字典存储到本地文件
with open('data/Afterprocess/questions_codenet_java.pkl', 'wb') as f:
    pickle.dump(num2questiondict, f)
#
with open('./data/Afterprocess/problems_codenet_java.pkl', 'wb') as f:
...
sequences2tl(split_and_pad_records(train_sequences,MAX_STEP), './data/Afterprocess/train_codenet_java.txt')
sequences2tl(split_and_pad_records(test_sequences,MAX_STEP), './data/Afterprocess/test_codenet_java.txt')